...
int vif_sum_s_blk(const float *x, int w, int h, int stride, int blk_x, int blk_y, int new_cols, int new_rows, float num_scale_blocks_row[new_cols*new_rows], int Nlines_x, int Nlines_y, int p_o, int left_over_x, int left_over_y)
{
    ...
    /*for (i = 0; i < h; ++i) {
	    ...
	    col_accum = MAX(floor(j / blk_y), 0);
            ...
            if ((col_accum < new_cols) && (row_accum < new_rows)) {
                num_scale_blocks_row[row_accum*new_cols + col_accum] +=  x[i * px_stride + j];
            }
    ...
    for (int block_start_x_case = 1; block_start_x_case < Nlines_x + 1; block_start_x_case++) {
		...
		for (int j = left_now; j <= right_now; ++j) {
		    ...
		    num_scale_blocks_row[Nlines_y * (block_start_x_case - 1) + block_start_y_case - 1] += val;
		}
	    }
        }
    }
