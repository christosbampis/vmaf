...
int all(const char *ref_path, const char *dis_path, int w, int h, const char *fmt, int start_x, int start_y, int roi_x, int roi_y, int blk_x_val, int blk_y_val, int p_o)
{
    double score = 0;
    double scores[4*2];
    double score_num = 0;
	...
	/*for (int i = 0; i < 4; i++) {
        ...
        if ((ret = compute_vif(ref_buf, dis_buf, w, h, stride, stride, &score, &score_num, &score_den, scores, frm_idx, Nscales, blk_x, blk_y, new_rows, new_cols, Nlines_x, Nlines_y, p_o, left_over_x, left_over_y)))
        {
